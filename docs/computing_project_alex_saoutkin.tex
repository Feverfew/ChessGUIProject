...
\section{Volumetrics}
In this particular application it is hard to estimate the memory needed to store the data needed to play a game. Likewise, it is hard to calculate the memory usage of the program without using a profiler. However, we can make some assumptions about the game and use this to estimate the memory required. Firstly the game is stored in a 
...
\begin{minted}[breaklines, breakanywhere, tabsize=4, baselinestretch=1.0]{python}
