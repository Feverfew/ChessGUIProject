...
\begin{figure}[H]
	...
	\node[rectangle, draw] (calculations) [below=of gameplay] {Game Calculations};
	\node[rectangle, draw] (promotions) [right=of calculations] {Manage Promotions};
	\node[rectangle, draw] (clicks) [left=of calculations] {Manage Table Clicks};
...
\begin{figure}[H]
	...
	\node[rectangle, draw] (filepath) [right=of load-game] {File Path};	
...
\end{figure}
\subsection{Third Level View}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\node[rectangle, draw] (clicks) {Manage Table Clicks};
	\node[rectangle, draw] (select-piece) [below left=of clicks] {Piece Selection};
	\node[rectangle, draw] (move-piece) [below right=of clicks] {Piece Movement};
	\draw [->] (clicks.south) -- (move-piece.north);
	\draw [->] (clicks.south) -- (select-piece.north);
	\end{tikzpicture}
	\caption{Third level hierarchy: Manage Table Clicks.}
\end{figure}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\node[rectangle, draw] (calculations) {Game Calculations};
	...
	\end{tikzpicture}
	\caption{Third level hierarchy chart: Game Calculations.}
\end{figure}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\node[rectangle, draw] (promotions) {Manage Promotions};
	\node[rectangle, draw] (piece-promote) [below=of promotions] {Choose Piece to Promote};
	\draw [->] (promotions.south) -- (piece-promote.north);
	\end{tikzpicture}
	\caption{Third level hierarchy chart: Manage Promotions.}
\end{figure}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\node[rectangle, draw] (save-game){Save Game};
	\node[rectangle, draw] (create-json) [below right=of save-game] {Create JSON};
	\node[rectangle, draw] (edit-json) [below left=of save-game] {Edit JSON};
	\draw [->] (save-game.south) -- (edit-json.north);
	\draw [->] (save-game.south) -- (create-json.north);
	\end{tikzpicture}
	\caption{Third level hierarchy chart: Save Game.}
\end{figure}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\node[rectangle, draw] (load-game) {Load Game};
	\node[rectangle, draw] (choose-game) [below left=of load-game] {Choose Game};
	\node[rectangle, draw] (sort-game) [below right=of load-game] {Sort Game};
	\draw [->] (load-game.south) -- (choose-game.north);
	\draw [->] (load-game.south) -- (sort-game.north);
	\end{tikzpicture}
	\caption{Third level hierarchy chart: Load Game.}
\end{figure}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\node[rectangle, draw] (filepath) {File Path};	
	\node[rectangle, draw] (filename) [below left=of filepath] {Choose Filename};
	\node[rectangle, draw] (directory) [below right=of filepath] {Choose Directory};
	\draw [->] (filepath.south) -- (filename.north);
	\draw [->] (filepath.south) -- (directory.north);
	\end{tikzpicture}
	\caption{Third level hierarchy chart: File Path.}
\end{figure}
\section{Code Design}
...
\subsection{Legal Move Calculation}
The algorithm for the calculation of legal moves requires the use of three functions. I have chosen to only show parts of the functions relating to the business logic of legal moves for the Rook class as these functons are fairly large and showing the business logic for calculating legal moves for the Rook class is sufficient to illustrate how the algorithm works.
\begin{figure}[H]
	\begin{minted}[breaklines, breakanywhere, tabsize=4, baselinestretch=1.0, fontsize=\footnotesize, linenos]{python}
	    def calculate_possible_moves(self):
			"""Calculates all the possible moves for a rook in a certain position."""
	...
	\end{minted}
	\caption{Source code for the functions which calculate the legal moves.}
	\label{calculate-possible-moves}
\end{figure}		
\begin{figure}[H]	
	\begin{minted}[breaklines, breakanywhere, tabsize=4, baselinestretch=1.0, fontsize=\footnotesize, linenos]{python}				
 		def get_attacking_moves(self, piece, board):
...
\begin{figure}[H]	
	...
	\end{minted}
	\caption{Source code for the functions which calculate the legal moves.}
	\label{calculate-legal-moves}
\end{figure}
In figure~\ref{calculate-possible-moves}, the function uses the position of the Rook to store all the possible moves (assuming there are no other pieces on the board) in the attribute \mintinline{python}/Rook.possible_moves/, akin to figure~\ref{rook-possible-moves}.

In figure~\ref{get-attacking-moves}, attacking moves are generated from the possible moves. This is done by removing any possible moves which are blocked by another piece. This function is called by the function in figure~\ref{calculate-legal-moves} which then takes out any attacking moves which contain a King, a piece of the same colour or that puts the moving player into check.
